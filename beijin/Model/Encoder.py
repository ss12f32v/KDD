...
class VanillaEncoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size):
        ...
        self.gru = nn.GRU(input_size= 806, 
    ...
    def forward(self, input_seqs, hidden=None):
        ...
        hour_embedded = self.hour_embedding(input_seqs[:, :, 6].long())
        day_embedded = self.day_embedding(input_seqs[:, :, 7].long())
        month_embedded = self.month_embedding(input_seqs[:, :, 8].long())
        year_embedded = self.year_embedding(input_seqs[:, :, 9].long())
        dayofweek_embedded = self.dayofweek_embedding(input_seqs[:, :, 10].long())
        dayofyear_embedded = self.dayofyear_embedding(input_seqs[:, :, 11].long())
        is_weekend_embedded = self.is_weekend_embedding(input_seqs[:, :, 12].long())
        work_time_embedded = self.work_time_embedding(input_seqs[:, :, 13].long())
        ...
        new_input_seqs = torch.cat((input_seqs[:, :, 0:6],one_hot_total_embedding), dim= 2)
...
class HighwayEncoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size):
        ...
        self.rhn = rhn.RecurrentHighwayNetwork(input_size=806, hidden_size=hidden_size, recurrence_length=7)
